...
zzwith <- plm(log(gsp)~log(pcap)+log(pc)+log(emp)+unemp,data=Produc)
@ 
...
<<>>=
zzra <- plm(log(gsp)~log(pcap)+log(pc)+log(emp)+unemp,data=Produc,model="random")
@ 
...
<<>>=
zzra <- plm(log(gsp)~log(pcap)+log(pc)+log(emp)+unemp,data=Produc,model="random",random.method="amemiya")
@ 
...
z <- plm(inv~value+capital,data=Grunfeld,model="random",effect="twoways",random.method="amemiya")
summary(z)
...
cr <- plm(form,data=Crime,model="random",instruments=inst,pvar=TRUE)

form2 <- log(crmrte)~log(prbarr)+log(polpc)+
  log(prbconv)+log(prbpris)+log(avgsen)+
  log(density)+log(wcon)+log(wtuc)+log(wtrd)+
  log(wfir)+log(wser)+log(wmfg)+log(wfed)+
  log(wsta)+log(wloc)+log(pctymle)+log(pctmin)+
  region+smsa+year | .-log(prbarr)-log(polpc)+log(taxpc)+log(mix)

cr1 <- plm(form,data=Crime,model="random",instruments=inst,pvar=TRUE)
...
<<>>=
#zz <- pggls(log(gsp)~log(pcap)+log(pc)+log(emp)+unemp,data=Produc,model="within")
#summary(zz)
@ 
...
<<>>=
waldtest(z,update(z,.~.-log(pf)-lf),vcov=function(x) pvcovHC(x,type="white2",weights="HC3"))
@ 
