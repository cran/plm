...
%\VignetteDepends{plm,stats,kinship,nlme}
...
long been common in other fields of statistic (where they are often
...
techniques econometricians themselves are used to: random and fixed
...
data management tasks as lagging, summing and, more in general,
...
The paper is organized as follows: Section~\ref{sec:linearpanelmodel}
...
Of course the latter is not estimable with $N = n \times T$ data points. A number
of assumptions are usually made about the parameters, the errors and
the exogeneity of the regressors, giving rise to a taxonomy of
...
$\epsilon_{it}$ is usually assumed well-behaved and independent from both the
regressors $x_{it}$ and the individual error component $\mu_i$. The
individual component may be in turn either independent from the
regressors or correlated.

If it is correlated, the ordinary least squares (\textsc{ols})
estimator for $\beta$ would be inconsistent, so it is customary to
...
is the most efficient estimator for $\beta$. This set of assumptions is
usually labelled {\em pooling} model, although this actually refers to
the errors' properties and the appropriate estimation method rather
...
Another way of estimating unobserved effects models through
removing time-invariant individual components is by first-differencing 
the data: lagging the model and subtracting, the time-invariant components
...
(where $\Delta y_{it}=y_{it}-y_{i,t-1}$, $\Delta x_{it}=x_{it}-x_{i,t-1}$ and, from
(\ref{eq:errcomp}), $\Delta u_{it}=u_{it}-u_{i,t-1}=\Delta \epsilon_{it}$ for $t=2,...,T$) 
can be consistently estimated by pooled \textsc{ols}. This is called
the {\em first-difference}, or \textsc{fd} estimator. Its relative efficiency,
and so reasons for choosing it against other consistent alternatives,
depends on the properties of the error term. The \textsc{fd} estimator
is usually preferred if the errors $u_{it}$ are strongly persistent in time,
because then the $\Delta u_{it}$ will tend to be serially uncorrelated.
...
Variable coefficients models relax the assumption that $\beta_{it}=\beta$ for all
$i,t$. Fixed coefficients models allow the coefficients to vary along
one dimension, like $\beta_{it}=\beta_i$ for all $t$. Random
coefficients models instead assume that coefficients vary randomly
around a common average, as $\beta_{it}=\beta+\eta_{i}$ for all $t$,
where $\eta_{i}$ is a group-- (time--) specific effect with mean zero.
...
argument is an ordinary \code{data.frame}, but an argument called 
\code{index} has to be added to indicate the structure of the data. 
This can be:
...
The \code{plm.data} function is then called, which returns a
\code{data.frame} with the two first columns containing the
...
For this reason the
three above estimators have been grouped inside the same function.
...
\code{plm} objects. This will be the subject of Subsection~\ref{subsec:inference}.
...
available in the \proglang{R} package \pkg{kinship}
...
residuals (both part of \code{plm} objects) is a consistent
estimator of the relevant model's parameters' covariance matrix, thus
the method is readily applicable to models estimated by random or
fixed effects, first difference or pooled \textsc{ols} methods. Different pre-weighting schemes taken
from package \pkg{sandwich} \citep{ZEIL:04} are also
implemented to improve small-sample performance. Robust estimators
with any combination of covariance structures and weighting schemes
can be passed on to the testing functions.
...
The four datasets used are \code{EmplUK} data available in \pkg{plm}
which was used by \citet{AREL:BOND:91}, the \code{Grunfeld} data
available in package \pkg{Ecdat} \citep{CROI:06} which is used in
several econometric books, the \code{Produc} data used by
\citet{MUNN:90} and the \code{Wages} used by \citet{CORN:RUPE:88}.
...
As an example, as the \code{Wages} dataset lacks indexes altogether,
but we know it is a balanced panel already sorted in (\code{id}, 
\code{time}) order, these can be explicitly reconstructed and put into
the first two columns by \code{plm.data} specifying the number of
individuals:
...
once for all. As an alternative, one could keep specifying \code{index = 595}
into every command.

Datasets that have indexes but are, or are suspected to be, in the
wrong order can be set in the same way, specifying the names of the 
\code{id} and \code{time} indexes instead of the number of individuals.
For the sake of illustration, here is an example on the \code{EmplUK}
dataset (although this is in the correct order already):
...
is of dimension $(T-1,T)$. Note that \proglang{R}'s \code{diff()}
and \code{lag()} functions don't compute correctly these
transformations for panel data because they are unable to identify
when there is a change in individual in the data. They have been
therefore rewritten in order to handle correctly panel data.

The \code{Between}, \code{between}, \code{within}, \code{lag}
and \code{diff} functions can be used directly in formulas to
estimate models in all the estimation functions previously described.

\end{itemize}

\subsection{Dynamic formula}

Using the \code{lag} and \code{diff} functions to write a formula
can be very cumbersome for dynamic models. A \code{dynformula}
function is provided to ease the writing of such formulas. Suppose for
example than one wants to estimate a model where employment depends on
its own first two lags and on the second and third lags of wages and
capital, all the variables except capital being in logs, and capital
being differenced. The formula is then:
<<results=hide>>=
log(emp)~lag(log(emp),1)+lag(log(emp),2)+lag(log(wage),2)+lag(log(wage),3)+diff(capital,2)+diff(capital,3)
@ 

which can be easily obtained with \code{dynformula}:

<<>>=
dynformula(emp~wage+capital,log=list(capital=FALSE,TRUE),lag=list(emp=2,c(2,3)),diff=list(FALSE,capital=TRUE))
@ 

The arguments \code{lag}, \code{diff} and \code{log} are lists which can be:

\begin{itemize}
\item unnamed, in this case, the length of the list must equal the
  number of variables,
\item named, in this case the missing variables get the default values
  (no lags, no logs, no differences),
\item partially named, in this case the unnamed element is the user
  defined default value.
\end{itemize}

The elements of \code{diff} and \code{log} lists should be logical
values. For \code{lag}, it is either a number or a vector of two
numbers: 3 means current value and first three lags, \code{c(1,2)}
means first two lags.
...
For a \code{random} model, the \code{summary} method gives
information about the variance of the components of the errors.
Fixed effects may be extracted easily  using  \code{fixef}:
...
As observed above, the random effect model is obtained as a linear estimation on
quasi-demeaned  data. The parameter of this transformation is
obtained using preliminary estimations. Four estimators of this
parameter are available, depending on the value of the argument \code{random.method} :
...
variables. The instruments are specified either as a one side formula
in the argument \code{instruments}, or at the end of the formula after
a | sign.
...
We illustrate instrumental variables estimation with the employment
equation. We use the first lag of wage as instrument of wage. The
four following commands are similar:
...
The Hausman-Taylor model \citep[see][]{HAUS:TAYL:81} may be estimated with \code{plm} by equating
the \code{model} argument to \code{"ht"}.
...
In a \textsc{gmm} estimation, there are ``normal'' instruments and ``\textsc{gmm}''
instruments. \textsc{gmm} instruments are indicated with the \code{gmm.inst}
argument (a one side formula) and the lags with the \code{lag.gmm}
argument. By default, all the variables of the model that are not used
as \textsc{gmm} instruments are used as normal instruments, with the same lag
structure.

The complete list of instruments can also be specified with the
argument \code{instruments} which should be a one side formula (or
\code{dynformula}).
...
\code{linear.hypothesis()} in package \pkg{car}, make it easy to
...
statistic is obtained by applying \code{linear.hypothesis()} to the
latter model to test the above restriction on $\delta$, supplying a
heteroskedasticity- and autocorrelation-consistent covariance matrix
(\code{pvcovHC} with the appropriate options, in particular
...
provided, mostly for use in Wald-type tests. \code{pvcovHC}
...
The main use of \code{pvcovHC} is together with testing functions from the
...
<<results=hide>>=
coeftest(re,pvcovHC(re,method="white2",type="HC3"))
@ 
...
Moreover, \code{linear.hypothesis} from package \pkg{car} may be used to test for linear restrictions:
...
A specific \code{pvcovHC} method for \code{pgmm} objects is also
